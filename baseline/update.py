...
@update.command()
@click.option(
    "-d", "--degree", required=True, type=click.Choice(["undergraduate", "graduate"])
)
@click.option(
    "-m",
    "--major",
    required=True,
    type=click.Choice(["general", "cs", "isee", "math", "physics", "se", "opteng"]),
)
@click.option(
    "-t", "--thesis_type", required=True, type=click.Choice(["thesis", "design"])
)
...
@click.option(
    ...
    required=True,
    default="master",
    type=click.Choice(["master", "doctor"]),
)
@click.option(
    "-l",
    "--language",
    required=True,
    default="chinese",
    type=click.Choice(["chinese", "english"]),
)
@click.option("-f", "--force", is_flag=True)
def single(degree, major, thesis_type, period, blind, grad_level, language, force):
    """Update single baseline pdf file"""
    update_single(
        degree, major, thesis_type, period, blind, grad_level, language, force
    )
...
def preset(force):
    ...
    # Blind Review
    update_single("undergraduate", "general", "thesis", "final", True, force=force)
    # Main Majors
    for m in major_main:
        for t in thesis_type:
            for p in periods:
                update_single("undergraduate", m, t, p, False, force=force)
    # CS Design Final and Proposal
    update_single("undergraduate", "cs", "design", "final", False, force=force)
    update_single("undergraduate", "cs", "design", "proposal", False, force=force)
    # Sample Majors
    for m in major_sample:
        update_single("undergraduate", m, "thesis", "final", False, force=force)
    # 2. Graduate
    update_single("graduate", "cs", "thesis", "final", False, "master", force=force)
    update_single(
        "graduate", "general", "thesis", "final", False, "master", force=force
    )
    update_single(
        "graduate", "general", "thesis", "final", False, "doctor", force=force
    )
    update_single("graduate", "opteng", "thesis", "final", False, "doctor", force=force)
    update_single(
        "graduate",
        "general",
        "thesis",
...
def update_single(
    ...
    with chmkdir(baseline_dir / "out"):
        run(
            [
                ...
                "clone",
                "--depth=1",
                "git@github.com:thenetadmin/zjuthesis",
                sub_dir,
            ],
    ...
    with chdir(baseline_dir / "out" / sub_dir):
        ...
        if (
            prev_cfg
            and prev_cfg["zjuthesis"]["commit_id"] == curr_cfg["zjuthesis"]["commit_id"]
            and not force
        ):
            print(
...
class texlive:
    ...
    def get_config(self) -> dict:
        ...
        res = run(
            ["docker", "run", "--rm", "adnrv/texlive:full", "latexmk", "--version"]
        )
        cfg["latexmk"] = res
        res = run(
            ["docker", "run", "--rm", "adnrv/texlive:full", "xelatex", "--version"]
        )
